"""
scripts/vault_test.py
---------------------
ONE-SHOT vault test on 2023-2024 OOS data using final_params.yaml.
DO NOT re-run after inspecting results — vault is a single look.

Usage:
    cd d:\raits\raits
    python raits/scripts/vault_test.py
"""
...
from raits.backtest.metrics import compute_metrics, check_vault_tier
...
WINDOWS = [
    ("2023-01-03", "2024-12-31", "2023-2024"),
]
...
def main():
    ...
    if args.use_results_cache and os.path.exists(PICKLE_RESULTS):
        ...
        cfg_base = dict(
            ...
            orb_range_minutes=20,   # final_params.yaml
            vwap_bb_std=1.5,        # final_params.yaml
            ema_period=30,          # final_params.yaml
        ...
        # ── Vault tier evaluation ─────────────────────────────────────────────
        import pandas as pd
        all_trades  = [t for r in results for t in r["trades"]]
        all_eq      = pd.concat([r["equity_curve"] for r in results if not r["equity_curve"].empty])
        vault_metrics = compute_metrics(all_eq, all_trades)
        tier = check_vault_tier(vault_metrics)
        print("\n" + "═"*60)
        print("  VAULT RESULT (2023–2024 OOS)")
        print("═"*60)
        print(f"  Tier:          {tier}")
        print(f"  Calmar:        {vault_metrics['calmar_ratio']:.2f}")
        print(f"  Sharpe:        {vault_metrics['sharpe_ratio']:.2f}")
        print(f"  Max DD:        {vault_metrics['max_drawdown']*100:.1f}%")
        print(f"  Profit Factor: {vault_metrics['profit_factor']:.2f}")
        print(f"  Win Rate:      {vault_metrics['win_rate']*100:.1f}%")
        print(f"  Total Return:  {vault_metrics['total_return']*100:.1f}%")
        print(f"  Total Trades:  {vault_metrics['total_trades']}")
        print("═"*60)
