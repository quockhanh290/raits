# Decision Unit — Task 4: Unit Tests

> **For agentic workers:** Use superpowers:subagent-driven-development to execute.
> Depends on Tasks 1 and 2.

**Goal:** Unit-test `DecisionUnit` in isolation — feed known context → assert known decision. Also test the mismatch detector (Task 5 prerequisite).

**Files:**
- Create: `raits/raits/tests/decision/test_decision_unit.py`

Run all with:
```
cd d:\raits\raits
pytest tests/decision/test_decision_unit.py -v
```

---

- [ ] **Step 1: Write `raits/raits/tests/decision/test_decision_unit.py`**
...
def _make_spy_bar(close: float = 450.0) -> pd.Series:
    return pd.Series({"open": 449.0, "high": 451.0, "low": 448.0, "close": close, "volume": 100_000})
...
class TestResetDay:
    ...
    def test_resets_strategies(self):
        ...
        du.trend.reset.assert_called_once()
...
class TestSafetyMode:
    def test_returns_exits_for_intraday_positions_when_override_active(self):
        du = _make_decision_unit()
        du.coordinator.trading_allowed = False  # triggers safety mode
    ...
    def test_safety_mode_does_not_close_tf_positions(self):
        ...
        assert all(e.trade is not tf_trade for e in res.exits)


# ── Tests: stop / target exits ────────────────────────────────────────────────

class TestExitDetection:
    def _run_bar_with_trade(self, trade, bar_ts_str, high, low, close, strategy_ok=True):
        du = _make_decision_unit()
...
class TestPositionLimits:
    ...
    def test_rejects_when_max_total_reached(self):
        du      = _make_decision_unit()
        trades  = [_make_trade(ticker=f"T{i}") for i in range(8)]  # MAX_TOTAL = 8
        pending = []
        assert not du._position_ok("NEW", "ORB", trades, pending)

    def test_rejects_when_strategy_cap_reached(self):
        du      = _make_decision_unit()
        trades  = [_make_trade(ticker=f"O{i}", strategy="ORB") for i in range(2)]  # MAX_ORB = 2
        pending = []
        assert not du._position_ok("NEW", "ORB", trades, pending)
    ...
    def test_counts_pending_entries(self):
        du      = _make_decision_unit()
        trades  = []
        pending = [EntryIntent("O0", "ORB", "LONG", 150, 100, 148, 154, True, "KELLY", "Normal"),
...
pytest tests/decision/test_decision_unit.py -v
```
Expected: all tests PASS. If any fail, fix `decision_unit.py` — do NOT weaken assertions.

- [ ] **Step 3: Commit**
```
git add tests/decision/test_decision_unit.py
git commit -m "test: unit tests for DecisionUnit (reset_day, decide, exits, position limits)"
```
