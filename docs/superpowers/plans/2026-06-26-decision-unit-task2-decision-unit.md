# Decision Unit — Task 2: DecisionUnit Class

> **For agentic workers:** Use superpowers:subagent-driven-development to execute.
> Depends on Task 1 (types).

**Goal:** Implement `DecisionUnit` — extracts the per-bar orchestration from `BacktestEngine._run_day` without touching `engine.py`.

**Files:**
- Create: `raits/raits/decision/decision_unit.py`

**Constraint:** `engine.py` is NEVER modified. All helper methods (e.g. `_compute_atr`) are duplicated as static methods here.
...
`DecisionUnit`:
- Owns strategy instances + risk instances passed at construction
- Owns intraday accumulated state: `or_ranges`, `pending_orb`, `_tf_cooldown`, etc.
- `reset_day(day, session_dt)` — called by engine at start of each day
- `decide(ctx: BarContext) -> DecisionResult` — called per bar; returns entries+exits, mutates `trade.stop` for trailing stops
- `on_trade_opened(trade, intent)` — engine calls this after opening a trade, so DecisionUnit can register GAP_FILL stop dists keyed by `id(trade)`

---

- [ ] **Step 1: Write `raits/raits/decision/decision_unit.py`**
...
class DecisionUnit:
    ...
    def decide(self, ctx: BarContext) -> DecisionResult:
        ...
        if bar_t == dtime(9, 30) and not self._pe_triggered:
            ...
            for _pe_ticker in _pe_today:
                ...
                if not self._position_ok(_pe_ticker, "PE_SHORT", ctx.open_trades, pending_entries):
                    continue
                _pe_day_stocks = ctx.day_stocks
                if _pe_ticker not in _pe_day_stocks:
                    _pe_bars_today = ctx.market_data[_pe_ticker][
                        ctx.market_data[_pe_ticker].index.normalize() == ctx.day
                    ]
                    if not _pe_bars_today.empty:
                        _pe_day_stocks = dict(_pe_day_stocks)
                        _pe_day_stocks[_pe_ticker] = _pe_bars_today
                if _pe_ticker not in _pe_day_stocks:
                    continue
                try:
                    _pe_today_bars = _pe_day_stocks[_pe_ticker]
                ...
                try:
                    ...
                    if sig:
                        if sig.get("direction") == "SHORT" and bar_t >= dtime(12, 30):
                            _spy_pre = [b for b in ctx.spy_history if pd.Timestamp(b.name) <= bar_ts]
                            if _spy_pre:
                                _spy_df = pd.DataFrame(_spy_pre)
                                _spy_vwap = self.vwap_mr.calculate_vwap(_spy_df)
                                if _spy_vwap and float(_spy_pre[-1]["close"]) > _spy_vwap:
                                    continue
        ...
        if bar_t == GAP_FILL_ENTRY and not self._gf_triggered and "GAP_FILL" in active:
            ...
            _spy_bars_pre = [b for b in ctx.spy_history if pd.Timestamp(b.name) <= bar_ts]
            _spy_df = pd.DataFrame(_spy_bars_pre) if _spy_bars_pre else pd.DataFrame()
            _spy_vwap = self.vwap_mr.calculate_vwap(_spy_df) if not _spy_df.empty else None
            _spy_above_vwap = (
                _spy_vwap is not None and _spy_bars_pre
                and float(_spy_bars_pre[-1]["close"]) > _spy_vwap
            )
            if _spy_above_vwap:
                for ticker in ctx.all_tickers:
                    ...
                    try:
                        ...
                        _before = len(entries) + len(pending_entries)
        ...
        if bar_t == GAP_FILL_ENTRY and not self._gfs_triggered and "GF_SHORT" in active:
            ...
            _spy_bars_pre_s = [b for b in ctx.spy_history if pd.Timestamp(b.name) <= bar_ts]
            _spy_df_s = pd.DataFrame(_spy_bars_pre_s) if _spy_bars_pre_s else pd.DataFrame()
            _spy_vwap_s = self.vwap_mr.calculate_vwap(_spy_df_s) if not _spy_df_s.empty else None
            _spy_below_vwap = (
                _spy_vwap_s is not None and _spy_bars_pre_s
                and float(_spy_bars_pre_s[-1]["close"]) < _spy_vwap_s
            )
            if _spy_below_vwap:
                for ticker in ctx.all_tickers:
                    if ticker == "SPY":
...
- [ ] **Step 3: Commit**
```
git add raits/decision/decision_unit.py
git commit -m "feat: implement DecisionUnit — extracts per-bar decision logic from BacktestEngine"
```
