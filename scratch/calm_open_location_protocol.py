...
def protocol(is_df: pd.DataFrame, variants: list[str], gate: Gate) -> tuple[pd.DataFrame, pd.DataFrame]:
    folds = [
    ...
    for fold, y0, y1, yt in folds:
        train = is_df[(is_df["year"] >= y0) & (is_df["year"] <= y1)]
        test = is_df[is_df["year"] == yt]
        ...
        eligible_train = train_rank[train_rank["eligible"]]
        selected = str(eligible_train.iloc[0]["variant"]) if not eligible_train.empty else "NONE"
        test_rank = []
        for variant in variants:
            g = test[test["variant"] == variant]
            test_rank.append({"variant": variant, **stats(g), **robustness(g)})
        test_rank = pd.DataFrame(test_rank).sort_values(["net", "pf", "avg"], ascending=False)
        test_rank["fold"] = fold
...
def print_folds(folds: pd.DataFrame) -> None:
    print("\n=== open-location protocol folds ===")
    for _, r in folds.iterrows():
        print(
            f"{r['fold']:<27} selected={r['selected']:<46} "
            f"test={int(r['test_year'])} n={int(r['selected_test_n']):>4} "
            f"net=${r['selected_test_net']:>8,.0f} pf={r['selected_test_pf']:>5.2f} "
            f"winner={r['test_winner']:<46} rank={int(r['selected_rank_by_net'])} "
...
def print_oos(title: str, df: pd.DataFrame, selected: list[str]) -> None:
    ...
    for variant in selected:
        ...
        rb = robustness(g)
        print(
            f"{variant:<46} n={st['n']:>4} days={st['days']:>3} "
...
def main() -> int:
    ...
    ap.add_argument("--is-trades", default="scratch/calm_open_location_drift_is.csv")
    ap.add_argument("--oos-2025", default="scratch/calm_open_location_drift_2025.csv")
    ap.add_argument("--oos-2026", default="scratch/calm_open_location_drift_2026.csv")
    ap.add_argument("--delay-2025", default="scratch/calm_open_location_drift_delay_sensitivity_2025.csv")
    ap.add_argument("--delay-2026", default="scratch/calm_open_location_drift_delay_sensitivity_2026.csv")
    ap.add_argument("--out-prefix", default="scratch/calm_open_location_protocol")
    ...
    gate = Gate()
    is_df = load(args.is_trades, variants)
    folds, ranks = protocol(is_df, variants, gate)
    print_folds(folds)
    ...
    if not selected:
        print("\nverdict=reject_protocol_no_selection")
        return 0

    o25 = pd.concat(
        [
            load(args.oos_2025, [v for v in selected if "_e0930_" in v]),
            load(args.delay_2025, [v for v in selected if "_e1000_" in v]),
        ],
        ignore_index=True,
    )
    o26 = pd.concat(
        [
            load(args.oos_2026, [v for v in selected if "_e0930_" in v]),
            load(args.delay_2026, [v for v in selected if "_e1000_" in v]),
        ],
        ignore_index=True,
    )
    print_oos("2025 OOS for protocol-selected variants", o25, selected)
    print_oos("2026 sanity for protocol-selected variants", o26, selected)
    pooled = pd.concat([o25, o26], ignore_index=True)
    print_oos("2025+2026 pooled", pooled, selected)

    winner_matches = int((folds["selected"] == folds["test_winner"]).sum())
    meaningful = int(folds["meaningful_test"].sum())
    if winner_matches >= 2 and meaningful == len(folds):
        print("\nverdict=protocol_survives_paper_candidate")
    else:
        print("\nverdict=keep_digging_protocol_not_decisive")
    return 0
