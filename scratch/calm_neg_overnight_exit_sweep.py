...
def run_window(args, data_dir: str, start: str, end: str, fit_end: str, calm_days_csv: str,
    ...
    e = pd.Timestamp(end)
    exits = ["10:00", "10:30", "11:00", "12:00", "13:00", "14:00", "15:55"]
    bands = [
        ("raw", -9.0, 0.0),
        ("mod001_008", -0.008, -0.001),
        ("mod002_008", -0.008, -0.002),
        ("mod001_010", -0.010, -0.001),
        ("mod002_010", -0.010, -0.002),
        ("mod003_010", -0.010, -0.003),
    ]
    for inst, contract in BASKET.items():
        ...
        for day, g in work.groupby("tday"):
            ...
            if not entry:
                continue
            exit_map = {hhmm: at_or_after(rth, hhmm, "open") for hhmm in exits}
            if any(v is None for v in exit_map.values()):
                continue
            ...
            for band_name, lo, hi in bands:
                ...
                for hhmm, ex in exit_map.items():
                    ...
                    variant = f"on_neg_fade_{band_name}_x{hhmm.replace(':', '')}"
                    ...
                    rows.append({
                        ...
                        "exit": float(exit_px),
                        "pnl": (float(exit_px) - float(entry_px)) * pv - cost,
                        "outside_exit_bar": outside_exit_bar,
                        "outside_entry_bar": outside_entry_bar,
...
def print_summary(df: pd.DataFrame, title: str, limit: int = 45) -> None:
    ...
    for _, r in rank.iterrows():
        print(f"{r.variant:<34} n={int(r.n):5d} pnl={r.pnl:9.0f} pf={r.pf:5.2f} "
...
def select_candidates(df: pd.DataFrame, limit: int = 2) -> list[str]:
    ...
    for v, g in df.groupby("variant"):
        ...
        audit_ok = int(g["outside_exit_bar"].sum()) == 0 and int(g["outside_entry_bar"].sum()) == 0 and int(g["signal_after_entry"].sum()) == 0
        if audit_ok and st["n"] >= 300 and st["pnl"] >= 5000 and st["pf"] >= 1.12 and (by_year > 0).sum() >= 5 and top_share <= 0.70 and (by_inst > 0).sum() >= 2:
...
def main() -> int:
    ...
    ap.add_argument("--out-prefix", default="scratch/calm_neg_overnight_exit_sweep")
    ...
    print_summary(is_df, "2018-2024 IS negative overnight exit sweep")
