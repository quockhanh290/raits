...
def spy_context(path: str) -> pd.DataFrame:
    ...
    out["spy_close_d1"] = close.shift(1).to_numpy()
    out["spy_sma50_d1"] = close.rolling(50).mean().shift(1).to_numpy()
...
def build_daily(data_dir: str, inst: str, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    ...
    s = (start - pd.Timedelta(days=30)).tz_localize(df.index.tz)
    ...
    for day, g in rth.groupby("day", sort=True):
        ...
        if day < start - pd.Timedelta(days=30) or day > end:
            continue
        if len(g) < 160:
            continue
        bars = {
            ...
            "1200": at_or_after(g, "12:00"),
            "1555": at_or_after(g, "15:55"),
        }
        if not all(bars.values()):
            continue
        b0930 = bars["0930"]
        assert b0930 is not None
        rows.append(
            {
                "day": day,
                "inst": inst,
                "rth_open": float(b0930[1]["open"]),
                "rth_high": float(g["high"].max()),
                "rth_low": float(g["low"].min()),
                "rth_close": float(g["close"].iloc[-1]),
                "t0930": b0930[0],
                "p0930": float(b0930[1]["open"]),
                "low0930": float(b0930[1]["low"]),
                "high0930": float(b0930[1]["high"]),
                "t1000": bars["1000"][0],
                "p1000": float(bars["1000"][1]["open"]),
                "low1000": float(bars["1000"][1]["low"]),
                "high1000": float(bars["1000"][1]["high"]),
                "t1200": bars["1200"][0],
                "p1200": float(bars["1200"][1]["open"]),
                "low1200": float(bars["1200"][1]["low"]),
                "high1200": float(bars["1200"][1]["high"]),
                "t1555": bars["1555"][0],
    ...
    out["open_loc"] = (out["p0930"] - out["prev_low"]) / denom
...
def build_rows(
    ...
    filters = {
        "all": lambda r: True,
        "lower_third": lambda r: r["open_loc"] <= 1.0 / 3.0,
        "upper_third": lambda r: r["open_loc"] >= 2.0 / 3.0,
        "prior_up": lambda r: r["prev_ret"] > 0,
        "prior_down": lambda r: r["prev_ret"] <= 0,
        "lower_prior_up": lambda r: (r["open_loc"] <= 1.0 / 3.0) and (r["prev_ret"] > 0),
        "upper_prior_down": lambda r: (r["open_loc"] >= 2.0 / 3.0) and (r["prev_ret"] <= 0),
        "lower_spy_above": lambda r: (r["open_loc"] <= 1.0 / 3.0) and bool(r["spy_above50_d1"]),
        "upper_spy_below": lambda r: (r["open_loc"] >= 2.0 / 3.0) and (not bool(r["spy_above50_d1"])),
    }
    for inst in sorted(instruments):
        ...
        for _, r in daily.iterrows():
            ...
            if labels.get(day) != "Calm":
                continue
            if pd.isna(r["open_loc"]) or pd.isna(r["prev_ret"]) or pd.isna(r["spy_above50_d1"]):
                continue
            for filt_name, filt in filters.items():
                ...
                for direction in ("LONG", "SHORT"):
                    for entry_key in ("0930", "1000"):
                        for exit_key in ("1200", "1555"):
                            if entry_key == "1000" and exit_key == "1200":
                                # Kept out to avoid a very short, lunch-noisy holding period.
                                continue
                            variant = f"openloc_{filt_name}_{direction.lower()}_e{entry_key}_x{exit_key}"
                            if selected is not None and variant not in selected:
                                continue
                            entry = float(r[f"p{entry_key}"])
                            exit_px = float(r[f"p{exit_key}"])
                            pnl_pts = exit_px - entry if direction == "LONG" else entry - exit_px
                            signal_ts = r["t0930"]
                            entry_ts = r[f"t{entry_key}"]
                            exit_ts = r[f"t{exit_key}"]
                            rows.append(
                                {
                                    "variant": variant,
                                    "inst": inst,
                                    ...
                                    "pnl": pnl_pts * pv - cost,
                                    "outside_exit_bar": 0
                                    if price_inside(exit_px, r[f"low{exit_key}"], r[f"high{exit_key}"])
                                    else 1,
                                    "outside_entry_bar": 0
...
def summarize(df: pd.DataFrame, window: str) -> pd.DataFrame:
    ...
    for v, g in df.groupby("variant"):
        ...
        top_share = float(by_year.max() / st["net"]) if st["net"] > 0 else 1.0
        rows.append(
            {
                "window": window,
                "variant": v,
...
def print_summary(title: str, summary: pd.DataFrame, variants: list[str] | None = None, limit: int = 30) -> None:
    print(f"\n=== {title} ===")
    if summary.empty:
        print("no rows")
        return
    tbl = summary if variants is None else summary[summary["variant"].isin(variants)]
...
def detail(title: str, df: pd.DataFrame, selected: list[str]) -> None:
    ...
    for v in selected:
        ...
        st = stats(g)
        print(
            f"\n{v} n={st['n']} days={st['days']} net=${st['net']:.0f} "
            f"pf={st['pf']:.2f} avg=${st['avg']:.2f} "
            f"audit={int(g['outside_exit_bar'].sum())}/{int(g['outside_entry_bar'].sum())}/"
            f"{int(g['signal_after_entry'].sum())}"
        )
        print("-- by instrument --")
...
def run_window(args: argparse.Namespace, window: str, data_dir: str, start: str, end: str, fit_end: str,
    ...
    summary = summarize(trades, window)
...
def main() -> int:
    ...
    ap.add_argument("--out-prefix", default="scratch/calm_open_location_drift")
    ...
    is_trades, is_summary = run_window(
        args, "is", args.data_dir, "2018-01-01", "2024-12-31", args.hmm_fit_end_is, args.calm_days_is
    )
    is_trades.to_csv(f"{args.out_prefix}_is.csv", index=False)
    is_summary.to_csv(f"{args.out_prefix}_is_summary.csv", index=False)
    print(f"wrote {args.out_prefix}_is.csv rows={len(is_trades)}")
    print_summary("2018-2024 IS Calm open-location drift", is_summary)
    ...
    detail("2018-2024 selected", is_trades, selected)

    o25_trades, o25_summary = run_window(
        args,
        "2025",
        args.data_dir_2025,
        "2025-01-01",
        "2025-12-31",
        args.hmm_fit_end_oos,
        args.calm_days_2025,
        set(selected),
    )
    o25_trades.to_csv(f"{args.out_prefix}_2025.csv", index=False)
    o25_summary.to_csv(f"{args.out_prefix}_2025_summary.csv", index=False)
    print(f"wrote {args.out_prefix}_2025.csv rows={len(o25_trades)}")
    detail("2025 OOS selected", o25_trades, selected)

    c26_trades, c26_summary = run_window(
        args,
        "2026",
        args.data_dir_2026,
        "2026-01-01",
    ...
    c26_trades.to_csv(f"{args.out_prefix}_2026.csv", index=False)
    c26_summary.to_csv(f"{args.out_prefix}_2026_summary.csv", index=False)
    print(f"wrote {args.out_prefix}_2026.csv rows={len(c26_trades)}")
    detail("2026 sanity selected", c26_trades, selected)

    pooled = pd.concat([o25_trades, c26_trades], ignore_index=True)
    ...
    survivors = []
    for v in selected:
        s25 = stats(o25_trades[o25_trades["variant"] == v])
        s26 = stats(c26_trades[c26_trades["variant"] == v])
        pooled_v = pooled[pooled["variant"] == v]
        pooled_s = stats(pooled_v)
        by_inst = pooled_v.groupby("inst")["pnl"].sum()
        if (
            s25["net"] > 0
            and s26["net"] > 0
            and pooled_s["pf"] >= 1.10
            and int((by_inst > 0).sum()) >= 2
            and int(pooled_v[AUDIT_COLS].sum().sum()) == 0
        ):
            survivors.append(v)
    if survivors:
