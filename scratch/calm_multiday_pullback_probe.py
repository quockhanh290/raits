...
def spy_context(path: str) -> pd.DataFrame:
    ...
    out["spy_2down_d1"] = ((ret.shift(1) < 0) & (ret.shift(2) < 0)).to_numpy()
    out["spy_3down_d1"] = ((ret.shift(1) < 0) & (ret.shift(2) < 0) & (ret.shift(3) < 0)).to_numpy()
...
def build_daily(data_dir: str, inst: str, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    ...
    s = (start - pd.Timedelta(days=60)).tz_localize(df.index.tz)
    ...
    for day, g in rth.groupby("day", sort=True):
        ...
        if day < start - pd.Timedelta(days=60) or day > end or len(g) < 160:
            continue
        bars = {"1000": at_or_after(g, "10:00"), "1555": at_or_after(g, "15:55")}
    ...
    out["ret"] = out["rth_close"] / out["rth_open"] - 1.0
    ...
    denom5 = (out["rth_high"].shift(1).rolling(5).max() - out["rth_low"].shift(1).rolling(5).min()).replace(0, np.nan)
    out["prev_close_loc_3d"] = (out["rth_close"].shift(1) - out["rth_low"].shift(1).rolling(3).min()) / denom3
    out["prev_close_loc_5d"] = (out["rth_close"].shift(1) - out["rth_low"].shift(1).rolling(5).min()) / denom5
    out["prev_2down"] = (out["close_ret"].shift(1) < 0) & (out["close_ret"].shift(2) < 0)
    out["prev_3down"] = out["prev_2down"] & (out["close_ret"].shift(3) < 0)
    out["prev_2sum"] = out["close_ret"].shift(1) + out["close_ret"].shift(2)
    out["prev_3sum"] = out["prev_2sum"] + out["close_ret"].shift(3)
    out["prev_range3_pct"] = (out["rth_high"].shift(1).rolling(3).max() - out["rth_low"].shift(1).rolling(3).min()) / out["rth_close"].shift(1)
    out["prev_range3_med20"] = out["prev_range3_pct"].rolling(20, min_periods=10).median().shift(1)
    return out[out["day"] >= start].copy()


def build_rows(data_dir: str, start: str, end: str, labels: dict, spy: pd.DataFrame,
               instruments: set[str], slippage_ticks: float, selected: set[str] | None = None) -> pd.DataFrame:
    filters = {
        "fut_2down": lambda r: bool(r["prev_2down"]),
        "fut_3down": lambda r: bool(r["prev_3down"]),
        "fut_2down_low3": lambda r: bool(r["prev_2down"]) and r["prev_close_loc_3d"] <= 0.25,
        "fut_3down_low5": lambda r: bool(r["prev_3down"]) and r["prev_close_loc_5d"] <= 0.35,
        "fut_2down_mild": lambda r: bool(r["prev_2down"]) and -0.025 <= r["prev_2sum"] <= -0.002,
        "fut_2down_low3_spyabove": lambda r: bool(r["prev_2down"]) and r["prev_close_loc_3d"] <= 0.25 and bool(r["spy_above50_d1"]),
        "fut_2down_low3_spy2down": lambda r: bool(r["prev_2down"]) and r["prev_close_loc_3d"] <= 0.25 and bool(r["spy_2down_d1"]),
        "fut_2down_low3_compress": lambda r: bool(r["prev_2down"]) and r["prev_close_loc_3d"] <= 0.25 and r["prev_range3_pct"] <= r["prev_range3_med20"],
    }
    ...
    for inst in sorted(instruments):
        ...
        for _, r in daily.iterrows():
            ...
            if labels.get(day) != "Calm":
                continue
            if pd.isna(r["prev_close_loc_3d"]) or pd.isna(r["spy_above50_d1"]):
                continue
            for name, fn in filters.items():
                if not fn(r):
                    continue
                for direction in ("LONG", "SHORT"):
                    variant = f"mdpb_{name}_{direction.lower()}_e1000_x1555"
                    ...
                    signal_ts = pd.Timestamp(r["day"]).tz_localize(r["t1000"].tz) - pd.Timedelta(minutes=1)
                    rows.append(
                        {
                            "variant": variant,
                            "inst": inst,
                            ...
                            "prev_close_loc_3d": float(r["prev_close_loc_3d"]),
                            "prev_2sum": float(r["prev_2sum"]) if pd.notna(r["prev_2sum"]) else np.nan,
...
def summarize(df: pd.DataFrame, window: str) -> pd.DataFrame:
    ...
    for variant, g in df.groupby("variant"):
        ...
        rows.append({"window": window, "variant": variant, **st,
                     "pos_years": int((by_year > 0).sum()), "years": int(len(by_year)),
...
def select_candidates(summary: pd.DataFrame, limit: int) -> list[str]:
    keep = summary[(summary["n"] >= 250) & (summary["net"] >= 5000) & (summary["pf"] >= 1.12)
                   & (summary["pos_years"] >= 5) & (summary["pos_inst"] >= 2)
...
def print_summary(title: str, summary: pd.DataFrame, selected: list[str] | None = None, limit: int = 30) -> None:
    ...
    for _, r in tbl.head(limit).iterrows():
        print(f"{r['variant']:<48} n={int(r['n']):>5} days={int(r['days']):>4} "
...
def run_window(args, window: str, data_dir: str, start: str, end: str, fit_end: str, calm_days_csv: str,
...
def main() -> int:
    ...
    ap.add_argument("--out-prefix", default="scratch/calm_multiday_pullback_probe")
    args = ap.parse_args()

    is_df, is_summary = run_window(args, "is", args.data_dir, "2018-01-01", "2024-12-31", args.hmm_fit_end_is, args.calm_days_is)
    ...
    print_summary("2018-2024 IS multi-day pullback", is_summary)
    ...
    o25, s25 = run_window(args, "2025", args.data_dir_2025, "2025-01-01", "2025-12-31", args.hmm_fit_end_oos, args.calm_days_2025, set(selected))
    ...
    c26, s26 = run_window(args, "2026", args.data_dir_2026, "2026-01-01", "2026-08-19", args.hmm_fit_end_oos, args.calm_days_2026, set(selected))
