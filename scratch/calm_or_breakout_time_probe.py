...
def build_rows(data_dir: str, start: str, end: str, instruments: set[str]) -> dict[str, pd.DataFrame]:
    ...
    for inst, contract in BASKET.items():
        ...
        for day_ts, day in df.groupby(df.index.normalize()):
            ...
            if not p0930 or not p1555:
                continue
            or60 = rth.between_time("09:30", "10:30")
            scan = rth[rth.index.time >= pd.Timestamp("10:31").time()]
            if len(or60) < 45 or len(scan) < 100:
                continue
            hi = float(or60["high"].max())
            lo = float(or60["low"].min())
            rng_pct = (hi - lo) / p0930[1]
            row = {"day": key, "p0930": p0930[1], "t1555": p1555[0], "p1555": p1555[1], "or60_pct": rng_pct}
            for direction, level, cmp_col, cmp_op in [
                ("LONG", hi, "high", ">="),
                ("SHORT", lo, "low", "<="),
            ]:
                cross = scan[scan[cmp_col] >= level] if cmp_op == ">=" else scan[scan[cmp_col] <= level]
                if cross.empty:
                    continue
                ...
                if later.empty:
                    continue
                ent = later.iloc[0]
                row[f"{direction}_signal"] = sig_ts
                row[f"{direction}_entry_ts"] = later.index[0]
                row[f"{direction}_entry"] = float(ent["open"])
            recs.append(row)
        out[inst] = pd.DataFrame(recs).set_index("day").sort_index()
    return out
    ...
    for inst, d in frames.items():
        ...
        for day, r in d.iterrows():
            ...
            for cap in (0.006, 0.008, 0.010, 0.012):
                if float(r["or60_pct"]) > cap:
                    continue
                for direction in ("LONG", "SHORT"):
                    if f"{direction}_entry" not in r or pd.isna(r.get(f"{direction}_entry")):
                        continue
                    for mode, trade_dir in [("breakout", direction), ("failed_breakout_fade", "SHORT" if direction == "LONG" else "LONG")]:
                        variant = f"calm_or60_{mode}_{direction.lower()}_cap{cap:g}_x1555"
                        if selected is not None and variant not in selected:
                            continue
                        entry = float(r[f"{direction}_entry"])
                        exit_px = float(r["p1555"])
                        pnl_pts = exit_px - entry if trade_dir == "LONG" else entry - exit_px
                        rows.append({
                            "variant": variant,
                            "inst": inst,
                            "direction": trade_dir,
                            "break_side": direction,
                            "day": day.date().isoformat(),
                            "year": int(day.year),
                            "signal_time": r[f"{direction}_signal"],
                            "entry_time": r[f"{direction}_entry_ts"],
                            "exit_time": r["t1555"],
                            "entry": entry,
                            "exit": exit_px,
                            "pnl": pnl_pts * pv - cost,
                            "outside_exit_bar": 0,
                            "or60_pct": float(r["or60_pct"]),
                        })
...
def print_summary(df: pd.DataFrame, title: str, limit: int = 40) -> None:
    ...
    for _, r in rank.iterrows():
        print(f"{r.variant:<56} n={int(r.n):5d} pnl={r.pnl:9.0f} pf={r.pf:5.2f} "
...
def main() -> int:
    ...
    ap.add_argument("--out-prefix", default="scratch/calm_or_breakout_time_probe")
    ...
    print_summary(is_df, "2018-2024 IS Calm OR60 breakout/fade time-exit")
