...
def build_rows(data_dir: str, start: str, end: str, instruments: set[str]) -> dict[str, pd.DataFrame]:
    ...
    for inst, contract in BASKET.items():
        ...
        for day_ts, day in df.groupby(df.index.normalize()):
            ...
            p1201 = at_or_after(rth, "12:01")
            p1400 = at_or_after(rth, "14:00")
            p1555 = at_or_after(rth, "15:55")
            if not all([p0930, p1200, p1201, p1400, p1555]):
                continue
            am = rth.between_time("09:30", "12:00")
            rows.append({
                "day": key,
                "t1200": p1200[0],
                "t1201": p1201[0],
                ...
                "p1400": p1400[1],
                "p1555": p1555[1],
                "am_ret": p1200[1] / p0930[1] - 1.0,
                "am_range_pct": float((am["high"].max() - am["low"].min()) / p0930[1]),
            })
    ...
    for inst, d in frames.items():
        ...
        for day, r in d.iterrows():
            ...
            if labels.get(day) != "Calm":
                continue
            ret = float(r["am_ret"])
            for min_abs in (0.0015, 0.0025, 0.0035, 0.0050):
                if abs(ret) < min_abs:
                    continue
                for cap in (0.008, 0.012, 0.016):
                    if float(r["am_range_pct"]) > cap:
                        continue
                    for mode in ("momo", "fade"):
                        ...
                        for exit_name, exit_col, exit_ts in [("1400", "p1400", "t1400"), ("1555", "p1555", "t1555")]:
                            variant = f"calm_midday_{mode}_thr{min_abs:g}_cap{cap:g}_x{exit_name}"
                            ...
                            rows.append({
                                ...
                                "outside_exit_bar": 0,
                                "am_ret": ret,
                                "am_range_pct": float(r["am_range_pct"]),
                            })
...
def print_summary(df: pd.DataFrame, title: str, limit: int = 40) -> None:
    ...
    for _, r in rank.iterrows():
        print(f"{r.variant:<50} n={int(r.n):5d} pnl={r.pnl:9.0f} pf={r.pf:5.2f} "
...
def main() -> int:
    ...
    ap.add_argument("--out-prefix", default="scratch/calm_midday_drive_probe")
    ...
    print_summary(is_df, "2018-2024 IS Calm midday drive")
