    ...
    def patched(df, labels, cost, *, ema_period=20, chandelier_atr_mult=3.0,
        ...
        if arm_hours is None:
            ...
            tr, _ = run_loop(df, labels, cost, strat=s, ema_period=ema_period,
                             ...
                             sig_cache=sig, stop_active_hour=arm_hours)
            H = arm_hours
        stat["trades"] += len(tr)
        if do_correct:
...
def main() -> int:
    runs = [("1 GOC (nguyen ban)", None, False),
            ("2 GOC + SUA KHOP LENH", None, True),
            ("3 LIVE 14h (tho)", 14.0, False),
            ("4 LIVE 14h + SUA KHOP LENH", 14.0, True)]
    saved = {}
    for tag, H, c in runs:
        print("\n" + "=" * 90)
        print("### " + tag)
