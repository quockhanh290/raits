...
try:
    from nonequity._core import load_parquet, FuturesCost as _FC
...
def main() -> None:
    ...
    print(f"session tz {a.tz} → entry window 14:00-15:55 = local power hour | "
          ...
          f"cost×{a.cost_mult:g} | REGIME-AGNOSTIC\n")
    print(f"data {df.index[0].date()} → {df.index[-1].date()}  ({len(df):,} bars)\n")

    trades = backtest_swing_tf(df, AllNormal(), cost, ema_period=a.ema_period,
